...
function DiaryCard({ entry, onEdit, onDelete, onView }: {
  entry: DiaryEntry
  onEdit: (entry: DiaryEntry) => void
  onDelete: (id: string) => void
  onView: (entry: DiaryEntry) => void
  ...
  const [photoMaxH, setPhotoMaxH] = useState<number | undefined>(undefined)
  const photos = entry.image_urls || []

  // Match photo column height to text column height
  useEffect(() => {
    const el = textRef.current
    if (!el || photos.length === 0) return

    const measure = () => setPhotoMaxH(el.clientHeight)
    measure()

    const obs = new ResizeObserver(measure)
    obs.observe(el)
    return () => obs.disconnect()
  }, [entry.id, photos.length])
  ...
  return (
    ...
    >
      <div className="flex gap-3 items-start">
        {/* Left: text content — determines the row height */}
        ...
        {/* Right: photos — constrained to text column height */}
        {photos.length > 0 && (
          <div
            className="shrink-0 grid gap-2 overflow-hidden"
            style={{
              maxWidth: '30%',
              maxHeight: photoMaxH ? `${photoMaxH}px` : undefined,
              gridTemplateRows: `repeat(${photos.length}, 1fr)`,
            }}
          >
            {photos.map((url, i) => (
